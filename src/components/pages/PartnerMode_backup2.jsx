...
const PartnerMode = ({ onBack }) => {
    ...
    const [showRankings, setShowRankings] = useState(false);    // refs - ClassicMode와 동일한 구조    const gameBoardRef = useRef(null);
        ...
        onValue(gameRef.current, (snapshot) => {
            ...
            if (gameData.gameBoard) {
                ...
                if (currentBoardString !== newBoardString) {
                    // 내가 현재 선택 중이 아닐 때만 보드 업데이트
                    if (!isSelecting && !mouseIsDownRef.current) {
                        ...
                        setTimeout(() => {
                            if (!isSelecting && !mouseIsDownRef.current) {
                                setGameBoard(gameData.gameBoard);
                                console.log('Delayed board update applied');
                            }
                        }, 1000);
                    }
                }
    ...
    const handleGlobalMouseUp = useCallback((e) => {
        ...
        // 선택 상태가 끝났음을 Firebase에 알림
        if (gameId.current && wasDown) {
    ...
    const handleMouseDown = (e) => {
        ...
        // 선택 상태를 즉시 Firebase에 전송 (로컬 상태 변경 전에)
        if (gameId.current) {
        ...
        // 선택 상자 생성
        createSelectionBox(x, y);        // 커서 위치 전송
        if (gameId.current) {
    ...
    const cleanupSelection = () => {
        ...
        // 먼저 상태를 정리한 다음 선택 확인
        const selectedCellsForCheck = [...selectedCells];
        cleanupSelection();
        
        // 선택 확인을 약간 지연시켜서 상태 정리가 완료되도록 함
        setTimeout(() => {
            checkSelectionWithCells(selectedCellsForCheck);
        }, 50);
    ...
    const checkSelection = async () => {
        ...
        if (sum === TARGET_SUM) {
            createPopSound();
            const newScore = score + selectedCells.length; // 사과 1개당 1점
            ...
            });            // Firebase에 업데이트 - 더 긴 지연을 두어 상대방의 선택 중단을 방지
            if (gameId.current) {
                try {
                    // 애니메이션이 완료되고 충분한 시간 후 보드 업데이트
                    setTimeout(async () => {
                        // 여전히 선택 중이 아닌지 다시 한번 확인
                        if (!isSelecting && !otherPlayerSelecting) {
                            ...
                            console.log('Board updated to Firebase after animation and verification');
                        } else {
                            console.log('Board update skipped - someone still selecting');
                        }
                    }, 500); // 애니메이션 시간보다 충분히 늦게
                } catch (error) {
        ...
        setSelectedCells([]);
    };    // 선택 확인 - 개선된 버전 (선택된 셀을 매개변수로 받음)
    const checkSelectionWithCells = async (cellsToCheck) => {
        if (!cellsToCheck || cellsToCheck.length < 2) return;

        const sum = cellsToCheck.reduce((total, cell) => total + cell.value, 0);
        console.log('선택된 셀:', cellsToCheck, '합계:', sum);

        if (sum === TARGET_SUM) {
            createPopSound();
            const newScore = score + cellsToCheck.length;
            console.log('점수 업데이트:', score, '->', newScore);
            ...
            if (gameId.current) {
                try {
                    await set(ref(database, `games/${gameId.current}/gameBoard`), newBoard);
                    await set(ref(database, `games/${gameId.current}/scores/${playerId.current}`), newScore);
                    console.log('Board and score updated to Firebase');
